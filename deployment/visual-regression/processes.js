...
function spawnAndWaitFor(cmd, args, readyString, logPrefix, env = {}, cwd = process.cwd()) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const stderrLog = fsSync.createWriteStream(`${logPrefix}.err.log`, { flags: "w" });
    ...
    proc.on("error", (error) => { console.error(`Process '${cmd} with args: ${args}' exited with ${error}`); reject(error) });

    proc.on("exit", (code) => {
      stdoutLog.end();
      stderrLog.end();
      if (!resolved) reject(new Error(`Process '${cmd} with args: ${args}' exited with code ${code}`));
    });
  });
}
...
async function startApi(jwtPublicKey, jwtPrivateKey, mediaServerUrl, dbConnectionString, logDir, workTreeDir, port) {
  console.log(`starting api on ${port}...`);
  return spawnAndWaitFor("npx", ["nx", "serve", "api-example", "--verbose"], "Public api is running on", `${logDir}/api-example`, {
...
async function startUi(medium, logDir, workTreeDir, port, apiPort) {
  ...
  return spawnAndWaitFor("npx", ["nx", "run", `${medium}:serve:production`, `--port=${port}`, "--verbose"], "Ready in", `${logDir}/ui`, {
    API_URL: apiUrl,
    NX_DAEMON: false // we don't want to be affected by daemon crashes
  },
    workTreeDir);
...
async function startProject(logDir, workTreeDir, dbConnectionString, medium, apiPort, uiPort) {
  const { JWT_PUBLIC_KEY, JWT_PRIVATE_KEY } = process.env;
  if (!JWT_PUBLIC_KEY || !JWT_PRIVATE_KEY) {
    throw new Error("JWT_PUBLIC_KEY and JWT_PRIVATE_KEY for the api server must be set as env variables");
  }
  await Promise.all([
  ...
  return Promise.all([startApi(JWT_PUBLIC_KEY, JWT_PRIVATE_KEY, mediaServerUrl, dbConnectionString, logDir, workTreeDir, apiPort), startUi(medium, logDir, workTreeDir, uiPort, apiPort)]);
}
