...
export const TeaserSlide = ({teaser, className}: Omit<BuilderTeaserProps, 'alignment'>) => {
  ...
  return (
    <TeaserSlideContainer className={className}>
      ...
      <TeaserSlideContent>
        <Paragraph css={metaStyles} gutterBottom={false}>
          {authors?.join(',')} {publishDate && <>am {date.format(new Date(publishDate), false)}</>}
        </Paragraph>

        <H5 css={titleStyles}>{title}</H5>
        <Paragraph css={leadStyles} gutterBottom={false}>
          {lead}
        </Paragraph>
