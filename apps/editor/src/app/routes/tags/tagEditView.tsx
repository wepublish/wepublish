...
import {MutationUpdateTagArgs, useTagQuery, useUpdateTagMutation} from '@wepublish/editor/api'
import {CanUpdateTag} from '@wepublish/permissions'
import {createCheckedPermissionComponent, SingleViewTitle} from '@wepublish/ui/editor'
...
const TagEditView = () => {
  ...
  return (
    <Form
      ...
      <TagForm
        tag={tag}
        onChange={changes => setTag(oldTag => ({...oldTag, ...(changes as any)}))}
      />
    </Form>
  )
}

const CheckedPermissionComponent = createCheckedPermissionComponent([CanUpdateTag.id])(TagEditView)
