...
export class ImageCacheService {
  ...
  private resetHitMissStat() {
    if (
      this.totalHits > this.maxCounter ||
      this.totalMisses > this.maxCounter
    ) {
  ...
  public get(key: string): [Buffer | undefined, number | undefined] {
    ...
    if (
      ...
      newTTL * 0.2 > (this.cache.getTtl(key) - now) / 1000
    ) {
  ...
  public state() {
    ...
    return {
      ...
      stats: {
        ...
        uptimeMin: Math.ceil(
          (new Date().getTime() - this.startDate.getTime()) / 1000 / 60
        ),
        ...
        freeCacheSizeMb: Math.ceil(
          this.maxCacheSizeMB - this.currentCacheSizeBytes / 1024 / 1024
        ),
        percentageFreeCacheItems: Math.ceil(
          this.cache.keys().length / this.maxCacheItems
        ),
        ...
        averageItemSizeKb:
          this.currentCacheSizeBytes / 1024 / this.cache.keys().length,
