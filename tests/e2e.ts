...
const createArticle = Selector('a').withAttribute('href', '/articles/create')
...
const metaPreTitleInput = Selector('.preTitle').child('input')
const metaTitleInput = Selector('.title').child('input')
const metaLeadInput = Selector('.lead').child('textarea')
...
const EDITOR_URL = process.env.BRANCH_NAME
  ? `https://editor.${slugify(process.env.BRANCH_NAME.substring(0, 12))}.wepublish.dev`
  : process.env.E2E_TEST_EDITOR_URL
const WEBSITE_URL = process.env.BRANCH_NAME
  ? `https://www.${slugify(process.env.BRANCH_NAME.substring(0, 12))}.wepublish.dev`
  : process.env.E2E_TEST_WEBSITE_URL
...
fixture`Create and publish an article`.disablePageCaching.beforeEach(async t => {
...
test('Check Front site', async t => {
  await t
    ...
    .expect(
      Selector('a')
        .withAttribute('href', 'https://www.facebook.com/wepublish')
        .getAttribute('target')
    )
    .eql('_blank')
})
...
test('Create an article', async t => {
  ...
  await t.click(createArticle)

  await t.expect(await getPath()).contains('/articles/create')
  ...
  await t.expect(path).contains('/articles/edit')

  await t
    .click(metadataButton)
    .expect(metaTitleInput.value)
    .contains('This is the article Title')
    .expect(metaLeadInput.value)
    .contains('This is the article lead')
    .typeText(metaPreTitleInput, 'This is a Pre-title')
    .click(closeButton)
})

test('Test Website', async t => {
  await t.navigateTo(`${WEBSITE_URL}/a/${articleID}`).expect(Selector('h1').innerText).eql('404')
})
...
test('Publish article', async t => {
  ...
  await t
    .click(Selector('a').withAttribute('href', `/articles/edit/${articleID}`))
    .click(publishButton)
    .expect(Selector('button').withAttribute('disabled').exists)
    .ok()
  ...
  await t
    ...
    .click(publishButton)
    .click(confirmButton)
    .expect(Selector('div.rs-tag-default').child('span.rs-tag-text').innerText)
    .contains('Article published')
})
...
test('Test Website', async t => {
  ...
  await t
    ...
    .expect(h1Title.innerText)
    .eql('This is the article Title')
})
