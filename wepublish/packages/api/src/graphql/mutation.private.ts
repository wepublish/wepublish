...
import {GraphQLOrganisation, GraphQLOrganisationInput} from './organisation'
...
export const GraphQLAdminMutation = new GraphQLObjectType<undefined, Context>({
  ...
  fields: {
    ...
    // =====
    createOrganisation: {
      type: GraphQLOrganisation,
      args: {input: {type: GraphQLNonNull(GraphQLOrganisationInput)}},
      resolve(root, {input}, {authenticate, dbAdapter}) {
        const {roles} = authenticate()
        authorise(CanCreateOrganisation, roles)
        return dbAdapter.organisation.createOrganisation({input})
      }
    },
  }
})
