...
import {
  GraphQLOrganisationConnection,
  GraphQLOrganisationFilter,
  GraphQLOrganisationSort,
} from './organisation'

import {OrganisationSort} from '../db/organisation'
...
export const GraphQLQuery = new GraphQLObjectType<undefined, Context>({
  ...
  fields: {
    ...
    organisations: {
      type: GraphQLNonNull(GraphQLOrganisationConnection),
      args: {
        after: {type: GraphQLID},
        before: {type: GraphQLID},
        first: {type: GraphQLInt},
        last: {type: GraphQLInt},
        skip: {type: GraphQLInt},
        filter: {type: GraphQLOrganisationFilter},
        sort: {type: GraphQLOrganisationSort, defaultValue: OrganisationSort.ModifiedAt},
        order: {type: GraphQLSortOrder, defaultValue: SortOrder.Descending}
      },
      resolve(
        root,
        ...
        const {roles} = authenticate()
        authorise(CanGetOrganisations, roles)

        return dbAdapter.organisation.getOrganisations({
          filter,
          sort,
          order,
          cursor: InputCursor(after, before),
