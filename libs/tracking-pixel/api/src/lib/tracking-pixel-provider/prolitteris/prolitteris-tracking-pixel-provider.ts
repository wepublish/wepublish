...
export class ProlitterisTrackingPixelProvider implements TrackingPixelProvider {
  private gateway = this.props.usePublisherInternalKey
    ? new InternalKey(this.props.memberNr, this.props.publisherInternalKeyDomain)
    : new GatewayClient(
        this.props.memberNr,
        this.props.username,
        this.props.password,
  ...
  private uriPaidContentIndicator = this.props.onlyPaidContentAccess ? 'pw' : 'na'
  ...
  async createPixelUri(internalTrackingId: string): Promise<PixelUrl> {
    const trackingPixels = await this.gateway.getTrackingPixels(internalTrackingId)
