...
export class KulturagendaParser {
  ...
  async fetchAndParseKulturagenda(urlToQuery: string, source: string): Promise<EventFromSource[]> {
    ...
    const upcomingEvents = events.filter((event: XMLEventType) => this.upcomingOnly(event))
