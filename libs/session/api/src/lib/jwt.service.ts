...
export class JwtService {
  constructor(
    @Inject(JWT_SECRET_KEY_TOKEN) private readonly jwtSecretKey: string,
    @Inject(HOST_URL_TOKEN) private readonly hostURL: string,
    @Inject(WEBSITE_URL_TOKEN) private readonly websiteURL: string
  ) {}

  generateJWT({id, expiresInMinutes = 60}: {id: string; expiresInMinutes?: number}): string {
  ...
  verifyJWT(token: string): string {
    ...
    try {
      ...
      return typeof decoded === 'object' && 'sub' in decoded ? (decoded.sub as string) : ''
    } catch (error) {
