...
import {PeerProfile, PeerProfileQuery, PeerProfileQueryVariables} from './remote/graphql'
  ...
  protected async loadByKeys(ids: string[]) {
    const profiles = await Promise.all(
      ids.map(async peerId => {
        ...
        const profile = await client.request<PeerProfileQuery, PeerProfileQueryVariables>(
          PeerProfile
        )
        ...
        return {
          ...
          callToActionImage: profile.peerProfile.callToActionImage
            ? {
                ...
                createdAt: new Date(profile.peerProfile.callToActionImage.createdAt),
                modifiedAt: new Date(profile.peerProfile.callToActionImage.modifiedAt)
              }
            : profile.peerProfile.callToActionImage
        }
      })
    ...
    return createOptionalsArray(ids, profiles, 'id') as unknown as Array<RemotePeerProfile | null>
  }
}
