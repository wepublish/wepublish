...
export const AverageRatingWrapper = styled('div')<Partial<RatingColors>>`
  .MuiRating-iconFilled {
    color: ${({theme, filledColor}) => filledColor || theme.palette.primary.main};
  }
...
export function Rating({
  ...
  const handleRatingComplete = (event: SyntheticEvent<Element, Event>, newValue: number | null) => {
  ...
  const displayValue = isRatingMode || internalRating > 0 ? internalRating : averageRating
  const isReadOnly = props.readOnly || !isRatingMode

  return (
    <RatingContainer onClick={handleContainerClick}>
      {!isRatingMode && internalRating === 0 ? (
        <AverageRatingWrapper filledColor={averageFilledColor} emptyColor={averageEmptyColor}>
          <MuiRating
            value={averageRating}
            icon={emptyIcon}
            emptyIcon={emptyIcon}
        ...
        <UserRatingWrapper filledColor={userFilledColor} emptyColor={userEmptyColor}>
          <MuiRating
            value={displayValue}
