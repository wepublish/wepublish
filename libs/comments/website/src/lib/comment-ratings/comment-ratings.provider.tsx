...
import {SettingName, useRateCommentMutation, useSettingListQuery} from '@wepublish/website/api'
...
const setAnonymousRate = (commentId: string, answerId: string, value: number) =>
  localStorage.setItem(`comment-rate:${commentId}:${answerId}`, value.toString())
...
export function CommentRatingsProvider({children}: PropsWithChildren) {
  ...
  const canRateAnonymously = useMemo(
    () =>
      !!settings?.settings.find(setting => setting.name === SettingName.AllowGuestCommentRating)
        ?.value,
    [settings?.settings]
  )
