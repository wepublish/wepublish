...
export const AsyncSessionProvider = memo<PropsWithChildren<{sessionToken: UserSession | null}>>(
  function SessionProvider({sessionToken, children}) {
    ...
    const setupInitialSessionToken = useCallback(async () => {
      ...
      const {sessionToken: token} = await (res.json() as Promise<{sessionToken: UserSession}>)
      setToken(token)
