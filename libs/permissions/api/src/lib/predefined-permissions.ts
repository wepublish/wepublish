...
>(
  ...
  switch (role.id) {
    case 'admin': {
      permissionIDs = [...role.permissionIDs, ...AllPermissions.map(permission => permission.id)]
      break
    }
    case 'editor': {
      permissionIDs = [...role.permissionIDs, ...EditorPermissions.map(permission => permission.id)]
      break
    }
    case 'peer': {
      permissionIDs = [...role.permissionIDs, ...PeerPermissions.map(permission => permission.id)]
      break
    }
  }
