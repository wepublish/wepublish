...
export class SubscriptionPropertyDataloader extends DataLoaderService<MetadataProperty[]> {
  ...
  protected async loadByKeys(subscriptionIds: string[]) {
    ...
    return subscriptionIds.map(subscriptionId => properties[subscriptionId] ?? [])
  }
}
