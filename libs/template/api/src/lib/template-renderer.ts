/**
 * Local template composition, shared by every channel.
 *
 * Templates are stored and edited inside wepublish and the variable
 * substitution happens here, locally, before the fully-composed message is
 * handed to a mail or letter provider. Provider-side template engines are not
 * used, so the same placeholders resolve identically in an email and on paper.
 */
 ...
 * the single source of truth for both {@link enrichTemplateData} and
...
export function resolvableKeys(data: Record<string, any>): string[] {
  return Object.keys(enrichTemplateData(data));
}
