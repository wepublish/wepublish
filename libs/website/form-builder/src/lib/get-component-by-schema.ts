...
export const getComponentBySchema = <Mapping extends FormSchemaMapping>(
  ...
  if (isArraySchema(input)) {
    if (input.item.type === 'object') {
      result =
        mapping.find(
          ([schema]) =>
            isArraySchema(schema) && input.item.entries === schema.item.entries
        ) ?? null;
    }
    ...
    if (!result) {
      result =
        mapping.find(
          ([schema]) =>
            isArraySchema(schema) && input.item.type === schema.item.type
        ) ?? null;
    }
  }
  ...
  if (isEnumSchema(input)) {
    result =
      mapping.find(
        ([schema]) => isEnumSchema(schema) && input.enum === schema.enum
      ) ?? null;
  }
  ...
  if (hasBrandAction(input)) {
    ...
    if (brand) {
      result =
        mapping.find(
          ([schema]) => hasBrandAction(schema) && brand === getBrandName(schema)
        ) ?? null;
    }
  }
  ...
  if (!result) {
    console.warn(
      `getComponentBySchema: Could not find component for schema`,
      input
    );
  }
