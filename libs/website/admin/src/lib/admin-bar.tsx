...
export const AdminBar = memo(() => {
  ...
  const canPreview = useMemo(() => user?.permissions.includes(CanPreview.id), [user?.permissions])
  const [isPreview, prevIsPreview, setIsPreview] = useSessionStorage(PREVIEW_MODE_KEY, {
    defaultValue: false,
  ...
  const actions = useMemo(() => {
    ...
    if (canPreview) {
      act.push({
        icon: <MdPreview size={24} color={isPreview ? theme.palette.primary.main : undefined} />,
