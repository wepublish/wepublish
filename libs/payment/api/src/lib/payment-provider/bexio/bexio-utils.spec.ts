...
import {searchForContact, addToStringReplaceMap, mapBexioStatusToPaymentStatus} from './bexio-utils'
...
describe('bexio-utils', () => {
  ...
  describe('searchForContact', () => {
    it('should search for contact by user email', async () => {
      ...
      ;(mockBexio.contacts.search as jest.Mock).mockResolvedValue([{id: 1, name: 'Test User'}])
  ...
  describe('addToStringReplaceMap', () => {
    it('should add rules to string replace map', () => {
      ...
      expect(mockStringReplaceMap.addRule).toHaveBeenCalledWith(':user.id:', 'user-1')
    })
  })

  describe('mapBexioStatusToPaymentStatus', () => {
    it('should correctly map Bexio status to payment status', () => {
      expect(mapBexioStatusToPaymentStatus(31)).toEqual(PaymentState.requiresUserAction)
      expect(mapBexioStatusToPaymentStatus(16)).toEqual(PaymentState.requiresUserAction)
      expect(mapBexioStatusToPaymentStatus(7)).toEqual(PaymentState.requiresUserAction)
