...
export class BexioPaymentProvider extends BasePaymentProvider {
  ...
  async bexioCreate(invoiceId: string, isRenewal: boolean, successURL = ''): Promise<Intent> {
    try {
      ...
      const contact = await searchForContact(this.bexio, invoice.subscription.user)
      const updatedContact = await this.createOrUpdateContact(contact, invoice.subscription.user)
      const newInvoice = await this.createInvoice(updatedContact, invoice, isRenewal)
