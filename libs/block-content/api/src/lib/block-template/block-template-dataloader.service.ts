import { Injectable, Scope } from '@nestjs/common';
import { BlockTemplate, PrismaClient } from '@prisma/client';
import { Primeable, createOptionalsArray } from '@wepublish/utils/api';
import DataLoader from 'dataloader';

@Injectable({
  scope: Scope.REQUEST,
})
export class BlockTemplateDataloaderService implements Primeable<BlockTemplate> {
  private dataloader = new DataLoader<string, BlockTemplate | null>(
    async (ids: readonly string[]) =>
      createOptionalsArray(
        ids as string[],
        await this.prisma.blockTemplate.findMany({
          where: {
            id: {
              in: ids as string[],
            },
          },
        }),
        'id'
      ),
    { name: 'BlockTemplateDataloader' }
  );

  constructor(private prisma: PrismaClient) {}

  public prime(
    ...parameters: Parameters<DataLoader<string, BlockTemplate | null>['prime']>
  ) {
    return this.dataloader.prime(...parameters);
  }

  public load(
    ...parameters: Parameters<DataLoader<string, BlockTemplate | null>['load']>
  ) {
    return this.dataloader.load(...parameters);
  }

  public loadMany(
    ...parameters: Parameters<
      DataLoader<string, BlockTemplate | null>['loadMany']
    >
  ) {
    return this.dataloader.loadMany(...parameters);
  }
}
