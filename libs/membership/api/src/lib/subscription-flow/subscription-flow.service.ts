...
export class SubscriptionFlowService {
  ...
  async createFlow(flow: SubscriptionFlowModelCreateInput) {
    ...
    if (await this.filterHasOverlap(flow.memberPlanId, flow)) {
      throw new BadRequestException("You can't create this flow because there is a filter overlap!")
    }
  ...
  async updateFlow(flow: SubscriptionFlowModelUpdateInput) {
    ...
    if (!originalFlow) {
      throw new NotFoundException(`The subscription flow '${flow.id}' could not be found.`)
    }

    if (await this.filterHasOverlap(originalFlow.memberPlanId, flow)) {
      throw new BadRequestException("You can't update this flow because there is a filter overlap!")
    }
