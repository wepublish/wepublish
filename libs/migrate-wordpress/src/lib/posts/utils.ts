...
export const hasEmptyBlocks = (article: ArticleQuery['article']) =>
  article &&
  article.blocks.some(
    b => b.__typename === 'RichTextBlock' && isSlateNodeEmpty(b.richText)
  );
...
export const createTimer = () => {
  ...
  const secondsLeft = () =>
    done && total ? (secondsElapsed() / done) * total : undefined;
...
const removeLeadingEmptyNodes = (nodes: SlateNode[]) =>
  nodes.reduce<SlateNode[]>(
    (nodes, node) =>
      nodes.length === 0 && isNodeEmpty(node) ? [] : [...nodes, node],
    []
  );
...
export async function transformHtmlToSlate(html: string): Promise<SlateNode[]> {
  const slate =
    ((await convertHtmlToSlate(html)) as unknown as SlateNode[]) ?? [];
