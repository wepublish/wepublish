...
export async function findImage({
  title,
  url,
}: Pick<EnsureImageProps, 'title' | 'url'>) {
...
export async function getImagesByTitle(title: string) {
  return (
    await privateClient.request<ImageListQuery, ImageListQueryVariables>(
      ImageList,
      {
        filter: title,
        take: 100,
...
async function uploadStream(stream: Readable, input: UploadStreamInput) {
  ...
  if (uploadResponse.data.errors) {
    throw new Error(
      JSON.stringify(uploadResponse.data.errors, undefined, '  ')
    );
  }
  return uploadResponse.data.data;
}

type UploadStreamInput = Omit<UploadImageInput, 'file' | 'filename'> & {
  filename: string;
};
