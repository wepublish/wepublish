...
const ensureTag = async ({
  name,
  main = false,
}: TagInput): Promise<Tag | undefined> => {
  const tag = name;
...
export async function ensureTags({
  tags,
}: PreparedArticleData): Promise<Tag[]> {
  return (await Promise.all(tags.map(t => ensureTag(t)))).filter(
    t => t
  ) as Tag[];
}
...
export async function createTag(
  variables: Omit<CreateTagMutationVariables, 'type'>
) {
  return (
    await privateClient.request<CreateTagMutation, CreateTagMutationVariables>(
      CreateTag,
      {
        ...variables,
...
export async function updateTag(variables: UpdateTagMutationVariables) {
  return (
    await privateClient.request<UpdateTagMutation, UpdateTagMutationVariables>(
      UpdateTag,
      variables
    )
  ).updateTag!;
