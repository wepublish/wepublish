...
const BASE_URL: string =
  process.env['WORDPRESS_URL'] || 'https://mannschaft.com';
...
async function init() {
  try {
    ...
    const nextJsCompatibleRoutes: NextJsRedirectsMap =
      transformData(originalRedirects);
    console.log('fetching wordpress posts...');
    const wpArticleRedirects: NextJsRedirectsMap =
      await getRedirectsFromWpArticles();
    console.log('resolve redirects...');
    const nextJsRoutes = resolveRedirects({
...
function getRedirectsFromPosts(posts: WordpressPost[]): NextJsRedirectsMap {
  ...
  for (const post of posts) {
    wpArticleRedirects = addRedirectToMap(
      wpArticleRedirects,
      post.link,
      post.link,
      'wp-posts'
