...
export async function migrateSubscription(
  user: UserListQuery['users']['nodes'][number],
  row: Row
) {
  ...
  const existingSubscription = user.subscriptions.find(
    s => s.memberPlan.id === memberPlan.id
  );
  if (existingSubscription) {
    console.debug(
      ' subscription exists ',
      [email, memberPlan.slug].join(' / ')
    );
...
async function findMemberPlanByRow(row: Row) {
  ...
  const currencyPlans = memberPlans.filter(
    plan => plan.currency === row.currency.toUpperCase()
  );

  const billFilteredPlans = currencyPlans.filter(plan =>
    bothTrueOrBothFalse(
      plan.name.includes('Bill'),
      row.productName.includes('Bill')
    )
  );
  const biancaFilteredPlans = billFilteredPlans.filter(plan =>
    bothTrueOrBothFalse(
      plan.name.includes('Bianca'),
      row.productName.includes('Bianca')
    )
  );
  const selectedMemberPlans = biancaFilteredPlans.filter(
    plan => !plan.name.includes('65+')
  );
